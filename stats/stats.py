...
def tracking_id(request):
    ...
    if track_id == None:
        track_id = request.session['tracking_id'] = random_string_generator(size=30)
    return track_id
...
def recommended_from_views(request):
    ...
    if viewed:
        itemviews = ItemView.objects.filter(item__in=viewed).values('tracking_id')
        t_ids = [v['tracking_id'] for v in itemviews]
        if t_ids:
            all_viewed = Item.objects.all().filter(itemview__tracking_id__in=t_ids)
            if all_viewed:
                other_viewed = ItemView.objects.filter(item__in=all_viewed).exclude(item__in=viewed)
                if other_viewed:
                    return Item.objects.all().filter(itemview__in=other_viewed).distinct()

def get_recently_viewed(request):
    t_id = tracking_id(request)
    views = ItemView.objects.filter(tracking_id=t_id).values('item_id').order_by('-date')[0:4]
...
def recommended_from_search(request):
    ...
    for word in common_words:
        results = search_words(word, queryset=Item.objects.all())
        for r in results:
...
def frequently_searched_words(request):
    ...
    searches = SearchTerm.objects.filter(tracking_id=tracking_id(request)).values('query').order_by('-search_date')[0:10]
